...
def variance(p, x):
    x1 = x
    x2 = x ** 2
    return expected_value(p, x2) - expected_value(p, x1) ** 2
...
C_hat = (C00 + C01*1j + C10*1j + C11)/sqrt(2.)
...
print("With Coin0 as starting point:")
pos = arange(P) #range(P+1)
print("Variance: {0}".format(variance(prob, pos)))
print("Expected Value: {0}".format(expected_value(prob, pos)))
...
for k in range(P):
    ...
    proj = M_hat_k.dot(psiN)
    prob[k] = proj.dot(proj.conjugate()).real
print("With Coin1 as starting point:")
pos = arange(P) #range(P+1)
print("Variance: {0}".format(variance(prob, pos)))
print("Expected Value: {0}".format(expected_value(prob, pos)))
#lets plot the result
